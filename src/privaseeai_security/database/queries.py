...
async def get_device_threat_summary(
    ...
    # Count total threats
    total_query = select(func.count(ThreatEvent.id)).where(
...
async def get_trending_threats(
    ...
    previous_counts = {
        row.threat_type: row.previous_count for row in previous_result.all()
    }
